...
:main
call :print_status "Building RuneLite..."

:: Build RuneLite
mvn clean compile -pl "!runelite-maven-plugin" -Dmaven.plugin.validation=false -q
if %errorlevel% neq 0 (
    call :print_error "Build failed during compile phase"
    exit /b 1
)

call :print_status "Packaging RuneLite..."
mvn package -pl "!runelite-maven-plugin" -Dmaven.plugin.validation=false -DskipTests -q
if %errorlevel% neq 0 (
    call :print_error "Build failed during package phase"
    exit /b 1
)
