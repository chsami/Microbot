...
public enum MLMPickaxes {
    ...
    public boolean hasRequirements(boolean weild) {
        boolean canUse = Rs2Player.getSkillRequirement(Skill.MINING, this.miningLevel);
        boolean canWeild = Rs2Player.getSkillRequirement(Skill.ATTACK, this.attackLevel);
        if (weild)
            return canUse && canWeild;
        return canUse;
    }
}
