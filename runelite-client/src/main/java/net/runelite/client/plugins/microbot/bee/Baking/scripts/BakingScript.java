package net.runelite.client.plugins.microbot.bee.Baking.scripts;
...
import net.runelite.client.plugins.microbot.bee.Baking.BakingConfig;
...
public class BakingScript extends Script {

    @Inject
    private Client client;  // Inject RuneLite's Client to access game objects

    private boolean cookingBread;

    public boolean run(BakingConfig config) {
        Microbot.enableAutoRunOn = false;
        String cookingItem = config.cookingItem();
    ...
    public void walkToBanker() {
        ...
        List<NPC> bankers = getBankerNPCs();  // Assuming this method retrieves all bankers
    ...
    public void openNearestBank() {
        if (!Rs2Bank.isOpen()) {
            NPC nearestBanker = getBankerNPC();  // Find the closest NPC that has "Bank" action
    ...
    public static List<NPC> getBankerNPCs() {
        return getNpcs()
    ...
    private void cookItem(String uncookedItemName) {
        ...
        sleepUntilTrue(() -> !hasRawItem(uncookedItemName) && !Rs2Player.isAnimating(3500)
                || Rs2Dialogue.isInDialogue() || Rs2Player.isWalking(), 500, 30000);
    }
