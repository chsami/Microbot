...
public class Rs2GameObject {
    ...
    public static boolean interact(TileObject tileObject, String action, boolean checkCanReach) {
        ...
        if (checkCanReach && net.runelite.client.plugins.microbot.util.gameobject.Rs2GameObject.hasLineOfSight(tileObject))
            return clickObject(tileObject, action);
    ...
    public static GameObject findObject(String objectName, boolean exact, int distance, boolean checkLineOfSight, WorldPoint anchorPoint) {
        return getGameObjects(nameMatches(objectName, exact), anchorPoint, distance).stream().filter(o -> !checkLineOfSight || net.runelite.client.plugins.microbot.util.gameobject.Rs2GameObject.hasLineOfSight(o)).findFirst().orElse(null);
    }
    ...
    public static GameObject findReachableObject(String objectName, boolean exact, int distance, WorldPoint anchorPoint, boolean checkAction, String action) {
        ...
        Predicate<GameObject> filter = o -> {
            if (!net.runelite.client.plugins.microbot.util.gameobject.Rs2GameObject.isReachable(o)) {
                return false;
            }
...
// MenuEntryImpl(getOption=Use, getTarget=Barrier, getIdentifier=43700, getType=GAME_OBJECT_THIRD_OPTION, getParam0=53, getParam1=51, getItemId=-1, isForceLeftClick=true, getWorldViewId=-1, isDeprioritized=false)
    ...
    public static GameObject getGameObject(String objectName, WorldPoint location, boolean exact) {
        return getGameObject(nameMatches(objectName, exact), location);
    }

    public static GameObject getGameObject(String objectName, List<WorldPoint> locations, boolean exact) {
        return getGameObjects(nameMatches(objectName, exact))
                .stream()
                .filter(obj -> locations.contains(obj.getWorldLocation()))
                .findFirst()
                .orElse(null);
    }

    public static GameObject getGameObject(String objectName, List<WorldPoint> locations) {
        return getGameObject(objectName, locations, false);
    }

    public static List<GameObject> getGameObjects(String objectName, List<WorldPoint> locations, boolean exact) {
        return getGameObjects(nameMatches(objectName, exact))
                .stream()
                .filter(obj -> locations.contains(obj.getWorldLocation()))
                .collect(Collectors.toList());
    ...
    public static List<GameObject> getGameObjects(String objectName, List<WorldPoint> locations) {
        return getGameObjects(objectName, locations, false);
    }

    public static GameObject getGameObject(List<Integer> objectIds, WorldPoint location) {
        Set<Integer> idSet = new HashSet<>(objectIds);
        return getGameObject(obj -> idSet.contains(obj.getId()), location);
    }

    public static GameObject getGameObject(Integer[] objectIds, WorldPoint location) {
        return getGameObject(Arrays.asList(objectIds), location);
    }

    public static GameObject getGameObject(int objectId, List<WorldPoint> locations) {
        return getGameObjects(obj -> obj.getId() == objectId)
                .stream()
                .filter(obj -> locations.contains(obj.getWorldLocation()))
                .findFirst()
                .orElse(null);
    ...
    public static GameObject getGameObject(List<Integer> objectIds, List<WorldPoint> locations) {
        Set<Integer> idSet = new HashSet<>(objectIds);
        return getGameObjects(obj -> idSet.contains(obj.getId()))
                .stream()
                .filter(obj -> locations.contains(obj.getWorldLocation()))
                .findFirst()
                .orElse(null);
    ...
    public static GameObject getGameObject(Integer[] objectIds, List<WorldPoint> locations) {
        return getGameObject(Arrays.asList(objectIds), locations);
    }

    public static List<GameObject> getGameObjects(List<Integer> objectIds, List<WorldPoint> locations) {
        Set<Integer> idSet = new HashSet<>(objectIds);
        return getGameObjects(obj -> idSet.contains(obj.getId()))
                .stream()
                .filter(obj -> locations.contains(obj.getWorldLocation()))
    ...
    public static List<GameObject> getGameObjects(Integer[] objectIds, List<WorldPoint> locations) {
        return getGameObjects(Arrays.asList(objectIds), locations);
    }
}