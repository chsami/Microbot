...
import net.runelite.client.plugins.microbot.util.grandexchange.models.*;
...
//					MenuEntryImpl(getOption=Customise, getTarget=, getIdentifier=2, getType=CC_OP, getParam0=15, getParam1=30474266, getItemId=-1, isForceLeftClick=false, getWorldViewId=-1, isDeprioritized=false)
    ...
    /**
     * Checks if there is an active buy offer for the specified item ID.
     *
     * @param itemId
     * @return
     */
    public static GrandExchangeOfferDetails hasBuyOffer(int itemId) {
        for (GrandExchangeSlots GrandExchangeSlot : GrandExchangeSlots.values()) {
            GrandExchangeOffer offer = Microbot.getClient().getGrandExchangeOffers()[GrandExchangeSlot.ordinal()];

            if (offer == null || offer.getItemId() == 0) {
                continue;
            }

            if (offer.getState() != GrandExchangeOfferState.BOUGHT && offer.getState() != GrandExchangeOfferState.BUYING) {
                continue;
            }

            if (offer.getItemId() == itemId) {
                return getOfferDetails(GrandExchangeSlot);
            }
        }
        return null;
    }
    ...
    /**
     * Checks if there is an active sell offer for the specified item ID.
     *
     * @param itemId
     * @return
     */
    public static GrandExchangeOfferDetails hasSellOffer(int itemId) {
        for (GrandExchangeSlots GrandExchangeSlot : GrandExchangeSlots.values()) {
            GrandExchangeOffer offer = Microbot.getClient().getGrandExchangeOffers()[GrandExchangeSlot.ordinal()];
            ...
            if (offer.getState() != GrandExchangeOfferState.SELLING && offer.getState() != GrandExchangeOfferState.SOLD) {
                continue;
            }
